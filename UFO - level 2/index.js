...
function handleSearchButtonClick() {
  ...
  filteredAddresses = dataSet.filter(function(address) {
    ...
    var addressDatetime = address.datetime.substring(0, filterDatetime.length).toLowerCase(); //datetime
    var addressCountry = address.country.substring(0, filterCountry.length).toLowerCase(); //cournty
    var addressShape = address.shape.substring(0, filterShape.length).toLowerCase(); //shape
    if (addressState === filterState && addressCity === filterCity && addressDatetime === filterDatetime 
      && addressCountry === filterCountry && addressShape === filterShape ) {
      return true;
    }
    return false;
  });
